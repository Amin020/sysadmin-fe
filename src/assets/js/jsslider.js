(function (factory) {
  ...
  slider.prototype = {
    ...
    init: function (options) {
      ...
      if (!self.initialed) {
        ...
        self.navItems
          ...
          .css(
            "width",
            self.navItems.length * self.getNavItemWidth() -
              parseInt(self.navItems.eq(0).css("marginRight"))
          )
          .css({ height: self.navItems.eq(0).height() });
        self.navWrap.css({
          width: self.navItems.parent().width,
          height: self.navItems.eq(0).height(),
        });
        self.navItems.css("position", "absolute");
        self.navItems.each(function () {
          $(this).css({
            top: 0,
            left: $(this).index() * self.getNavItemWidth(),
          });
        });
        self.navItems
          .eq(self.navItems.length - 1)
        ...
        if (self.opts.event == "hover") {
          self.navItems.bind("mouseover", function () {
            ...
            self
              .slideQueueAdd(_index, false, self.getSlidTimeout())
              .slideQueueExec();
          });
        }
        self.navItems
        ...
        self.prevOneBtn
          ...
          .click(function () {
            self
              .slideQueueAdd(self.getPrevIndex(), false, self.getSlidTimeout())
              .slideQueueExec();
          });
        self.nextOneBtn
          .mouseenter(function () {
          ...
          .click(function () {
            self
              .slideQueueAdd(self.getNextIndex(), false, self.getSlidTimeout())
              .slideQueueExec();
          });
        self.prevPageBtn
          .mouseenter(function () {
    ...
    getLength: function () {
      ...
      return self.initialed
        ? self.length
        : self._slider.find("." + self.opts.banItemClass).length;
    },
    /* 设置当前索引 */
    ...
    getCurIndex: function () {
      var self = this;
      var _index =
        typeof self.curIndex == "undefined" ? 0 : parseInt(self.curIndex);
    ...
    getNavItemWidth: function () {
      ...
      var _margin_left = parseFloat(
        $(self.navItems).eq(0).css("margin-inline-end")
      );
      _margin_left = isNaN(_margin_left) ? 0 : _margin_left;
      var _margin_right = parseFloat(
        $(self.navItems).eq(0).css("margin-inline-start")
      );
      _margin_right = isNaN(_margin_right) ? 0 : _margin_right;
      var _padding_left = parseFloat(
        $(self.navItems).eq(0).css("padding-left")
      );
      _padding_left = isNaN(_padding_left) ? 0 : _padding_left;
      var _padding_right = parseFloat(
        $(self.navItems).eq(0).css("padding-right")
      );
      _padding_right = isNaN(_padding_right) ? 0 : _padding_right;
      var _border_left_width = parseFloat(
        $(self.navItems).eq(0).css("border-left-width")
      );
      _border_left_width = isNaN(_border_left_width) ? 0 : _border_left_width;
      var _border_right_width = parseFloat(
        $(self.navItems).eq(0).css("border-right-width")
      );
      _border_right_width = isNaN(_border_right_width)
        ? 0
        : _border_right_width;
    ...
    getOffset: function () {
      var self = this;
      var _offset =
        typeof self.offset == "undefined" ? 0 : parseInt(self.offset);
    ...
    getPages: function () {
      ...
      return self.initialed
        ? self.pages
        : Math.ceil(self.getLength() / self.opts.perpage);
    },
    /* 设置总页数 */
    ...
    getCurPage: function () {
      var self = this;
      var _curPage =
        typeof self.curPage == "undefined" ? 1 : parseInt(self.curPage);
    ...
    page: function (_page, hasSilde) {
      var self = this;
      var hasSilde =
        typeof hasSilde == "undefined" ? false : hasSilde ? true : false;
      ...
      r_offset =
        ...
        (_page - _curPage) *
        self.opts.perpage *
        _navItemWidth; /* 计算相对偏移 */
      _offset += r_offset;
      var maxOffset = 0,
        minOffset = -1 * (self.getLength() - self.opts.perpage) * _navItemWidth;
      _offset =
        _offset > maxOffset
          ? maxOffset
          : _offset < minOffset
      ...
      /* 翻页动作 */
      $(self.navItems).each(function () {
        $(this).css("left", _offset + $(this).index() * self.getNavItemWidth());
      });
      ...
      if (!hasSilde) {
        ...
        if (
          (_page > _curPage && _index <= self.getCurIndex()) ||
          (_page < _curPage && self.getCurIndex() - _index < opts.perpage)
        )
          _index = self.getCurIndex();
        self
          .slideQueueAdd(_index, true, self.getSlidTimeout())
          .slideQueueExec();
      }
    ...
    getIndexInPage: function (_index) {
      ...
      leftIndex =
        self.getCurIndex() - Math.ceil(leftOffset / self.getNavItemWidth());
      if (_index >= leftIndex && _index < leftIndex + self.opts.perpage) {
        _indexInPage = self.getCurPage();
      } else if (_index < leftIndex) {
        _indexInPage =
          self.getCurPage() -
          Math.ceil((leftIndex - _index) / self.opts.perpage);
        _indexInPage = _indexInPage < 1 ? 1 : _indexInPage;
      } else {
        _indexInPage =
          self.getCurPage() +
          Math.ceil(
            (_index - leftIndex - self.opts.perpage + 1) / self.opts.perpage
          );
        _indexInPage =
          _indexInPage > self.getPages() ? self.getPages() : _indexInPage;
      }
    ...
    slid: function (index, hasPage) {
      var self = this;
      var hasPage =
        typeof hasPage == "undefined" ? false : hasPage ? true : false;
      var index =
      ...
      var _nextIndexInPage = self.getIndexInPage(
        _nextIndex
      ); /* 下一个索引所在页 */
      ...
      if (self.opts.effect == "fade") {
        ...
        self.banItems.eq(_curIndex).fadeOut(400, function () {
          self.sliding = false;
        }); // fast:200, slow:600, default:400
      } else {
        self.banItems.hide();
        self.banItems.eq(_nextIndex).show(0, function () {
          self.sliding = false;
        });
      }
  ...
  $.fn.slider.defaults = {
    ...
    event:
      "hover" /* 切换事件,默认为鼠标经过切换,允许的值[hover:经过,click:单击] */,
    effect:
      "none" /* 切换效果,默认值none,允许的值[none:无效果,fade:淡入淡出] */,
